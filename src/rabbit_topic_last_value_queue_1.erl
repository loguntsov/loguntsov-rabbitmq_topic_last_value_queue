-module(rabbit_topic_last_value_queue_1).
...
-export([
  init/3, terminate/2, delete_and_terminate/2, delete_crashed/1,
...
-rabbit_boot_step({?MODULE,
  [{description, "enable last value queue"},
    {mfa,         {?MODULE, enable, []}},
    {requires,    rabbit_priority_queue},
...
enable() ->
  ...
  case RealBQ of
    ?MODULE ->
      rabbit_log:error("This module should be used as proxy queue module for other module. Pls change backing_queue_module key in settings", []),
      error(bad_settings);
    _ ->
      ok
  end,
  proc_lib:spawn_link(fun() ->
    rabbit_log:info("Last topic queue is starting, real BQ is ~s~n", [RealBQ]),
    { ok, _ } = application:ensure_all_started(rabbit_tlvq_app:app()),
    rabbit_tlvq_app:set_env(backing_queue_module, RealBQ),
    application:set_env(rabbit, backing_queue_module, ?MODULE),
    rabbit_log:info("Last topic queue was started, real BQ is ~s~n", [RealBQ])
  end),
  ok.

disable() ->
  RealBQ = rabbit_tlvq_app:get_env(backing_queue_module),
  proc_lib:spawn(fun() ->
    application:set_env(rabbit, backing_queue_module, RealBQ)
  end),
  ok.

is_last_topic_queue(Q) when ?is_amqqueue(Q) ->
  Args = amqqueue:get_arguments(Q),
  R = case rabbit_misc:table_lookup(Args, <<"x-last-value-per-topic">>) of
    {_, <<"enabled">>} -> true;
    undefined -> false
  end,
  %%rabbit_log:info("Q:~p=~p", [ Q, R ]),
  R.
...
bq() ->
  rabbit_tlvq_app:get_env(backing_queue_module).
...
%%    mailbox.
% -callback init(amqqueue:amqqueue(), recovery_info(), async_callback()) -> state().
init(Queue, Recover, Callback) ->
  case is_last_topic_queue(Queue) of
    false ->
      BQ = bq(),
      #passthrough{
        bq = BQ,
        bqs = BQ:init(Queue, Recover, Callback)
      };
    true ->
      case amqqueue:get_exclusive_owner(Queue) of
        none -> error({ queue_must_be_exclusive, amqqueue:get_name(Queue) });
...
delete_crashed(Q) ->
  rabbit_log:info("delete_crashed(~p)", [ Q ]),
  BQ = bq(),
  case is_last_topic_queue(Q) of
    false -> BQ:delete_crashed(Q);
    true ->
...
purge_acks(State) ->
  %%rabbit_log:info("purge_acks"),
  a(purge_pending_ack(false, State)).

%% Publish a message.
% -callback publish(rabbit_types:basic_message(), rabbit_types:message_properties(), boolean(), pid(), flow(), state()) -> state().
publish(Msg, MsgProps, IsDelivered, ChPid, Flow, State = #passthrough{bq = BQ, bqs = BQS}) ->
  ?passthrough1(publish(Msg, MsgProps, IsDelivered, ChPid, Flow, BQS));

publish(Msg, MsgProps, IsDelivered, ChPid, Flow, State) ->
  %%rabbit_log:info("publish(~p, ~p, ~p, ~p, ~p)", [ Msg, MsgProps, IsDelivered, ChPid, Flow ]),
  State1 =
...
batch_publish(Publishes, ChPid, Flow, State = #passthrough{bq = BQ, bqs = BQS}) ->
  ?passthrough1(batch_publish(Publishes, ChPid, Flow, BQS));

batch_publish(Publishes, ChPid, Flow, State) ->
  %%rabbit_log:info("batch_publish(~p, ~p, ~p)", [ Publishes, ChPid, Flow ]),
  {ChPid, Flow, State1} =
...
% -callback publish_delivered(rabbit_types:basic_message(), rabbit_types:message_properties(), pid(), flow(), state()) -> {ack(), state()}.
publish_delivered(Msg, MsgProps, ChPid, Flow, State = #passthrough{bq = BQ, bqs = BQS}) ->
  ?passthrough2(publish_delivered(Msg, MsgProps, ChPid, Flow, BQS));
publish_delivered(Msg, MsgProps, ChPid, Flow, State) ->
  %%rabbit_log:info("publish_delivered(~p, ~p, ~p, ~p)", [ Msg, MsgProps, ChPid, Flow ]),
  {SeqId, State1} =
...
% -callback batch_publish_delivered([delivered_publish()], pid(), flow(), state()) -> {[ack()], state()}.
batch_publish_delivered(Publishes, ChPid, Flow, State = #passthrough{bq = BQ, bqs = BQS}) ->
  ?passthrough2(batch_publish_delivered(Publishes, ChPid, Flow, BQS));
batch_publish_delivered(Publishes, ChPid, Flow, State) ->
  %%rabbit_log:info("batch_publish_delivered(~p, ~p, ~p)", [ Publishes, ChPid, Flow ]),
...
discard(_MsgId, _ChPid, _Flow, State) ->
  %%rabbit_log:info("discard(~p, ~p, ~p)", [ _MsgId, _ChPid, _Flow ]),
  State.
...
drain_confirmed(State = #vqstate { confirmed = C }) ->
  %%rabbit_log:info("drain_confirmed"),
...
% -callback dropwhile(msg_pred(), state()) -> {rabbit_types:message_properties() | undefined, state()}.
dropwhile(Pred, State = #passthrough{bq = BQ, bqs = BQS}) ->
  ?passthrough2(dropwhile(Pred, BQS));
dropwhile(Pred, State) ->
  %%rabbit_log:info("dropwhile()"),
  {MsgProps, State1} =
...
%% accumulator.
% -callback fetchwhile(msg_pred(), msg_fun(A), A, state()) -> {rabbit_types:message_properties() | undefined, A, state()}.
fetchwhile(Pred, Fun, Acc, State = #passthrough{bq = BQ, bqs = BQS}) ->
  ?passthrough3(fetchwhile(Pred, Fun, Acc, BQS));
fetchwhile(Pred, Fun, Acc, State) ->
  %%rabbit_log:info("fetchwhile(~p)", [ Acc ]),
...
% -callback drop
%     (true,  state()) -> {drop_result(ack()), state()};
...
drop(AckRequired, State) ->
  %%rabbit_log:info("drop(~p)", [ AckRequired]),
  case queue_out(State) of
    {empty, State1} ->
      {empty, a(State1)};
    {{value, MsgStatus}, State1} ->
...
requeue(AckTags, #vqstate { mode       = default,
  ...
  len        = Len } = State) ->
  %%rabbit_log:info("requeue(~p)", [ AckTags ]),
...
ackfold(MsgFun, Acc, State, AckTags) ->
  %%rabbit_log:info("ackfold(~p,~p)", [ Acc, AckTags]),
  {AccN, StateN} =
    lists:foldl(fun(SeqId, {Acc0, State0}) ->
      {value, MsgStatus } = lookup_pending_ack(SeqId, State0),
...
% -callback fold(fun((rabbit_types:basic_message(), rabbit_types:message_properties(), boolean(), A) ->
...
% -callback is_duplicate(rabbit_types:basic_message(), state()) -> {{true, drop} | {true, reject} | boolean(), state()}.
...
% -callback zip_msgs_and_acks([delivered_publish()], [ack()], Acc, state()) -> Acc.
...
convert_to_lazy(State) ->
  ...
  case Delta#delta.count + ltq_queue:len(Q3) == Len of
    true ->
      State1;
    false ->
...
get_q_head(Q) ->
  get_collection_head(Q, fun ltq_queue:is_empty/1, fun ltq_queue_peek/1).
...
a(State = #vqstate { q1 = Q1, q2 = Q2, delta = Delta, q3 = Q3, q4 = Q4,
  ...
  ram_msg_count    = RamMsgCount,
  ram_bytes        = RamBytes}) ->
  E1 = ltq_queue:is_empty(Q1),
  E2 = ltq_queue:is_empty(Q2),
  ED = Delta#delta.count == 0,
  E3 = ltq_queue:is_empty(Q3),
...
msg_status(IsPersistent, IsDelivered, SeqId,
  ...
  #msg_status{seq_id        = SeqId,
    msg_id        = MsgId,
    key           = Msg#basic_message.routing_keys,
    msg           = Msg,
    is_persistent = IsPersistent,
...
msg_status_key(#msg_status{ key = Key }) -> Key.
...
betas_from_index_entries(List, TransientThreshold, DelsAndAcksFun, State) ->
  {Filtered, Delivers, Acks, RamReadyCount, RamBytes, TransientCount, TransientBytes} =
    lists:foldr(
        ...
        case SeqId < TransientThreshold andalso not IsPersistent of
          ...
          false -> MsgStatus = m(beta_msg_status(M)),
            ...
            case is_msg_in_pending_acks(SeqId, State) of
              false -> {
                ltq_queue_in_r(MsgStatus, Filtered1),
                Delivers1, Acks1,
                RRC + one_if(HaveMsg),
      ...
      end, {ltq_queue:new(), [], [], 0, 0, 0, 0}, List),
...
in_r(MsgStatus = #msg_status { msg = undefined }, State = #vqstate { mode = default, q3 = Q3, q4 = Q4 }) ->
  case ltq_queue:is_empty(Q4) of
    true  ->
      State#vqstate{
        q3 = ltq_queue_in_r(MsgStatus, Q3)
      };
    false ->
      {Msg, State1 = #vqstate{ q4 = Q4a }} = read_msg(MsgStatus, State),
      MsgStatus1 = MsgStatus#msg_status{msg = Msg},
      NewState = State1#vqstate{
        q4 = ltq_queue_in_r(MsgStatus1, Q4a)
      },
      stats(ready0, {MsgStatus, MsgStatus1}, 0, NewState )
  end;
in_r(MsgStatus, State = #vqstate { mode = default, q4 = Q4 }) ->
  State#vqstate{ q4 = ltq_queue_in_r(MsgStatus, Q4) };
%% lazy queues
in_r(MsgStatus = #msg_status { seq_id = SeqId, is_persistent = IsPersistent },
    State = #vqstate { mode = lazy, q3 = Q3, delta = Delta}) ->
  case ltq_queue:is_empty(Q3) of
    true  ->
      {_MsgStatus1, State1} =
      ...
      State2 #vqstate{ delta = Delta1};
    false ->
      State #vqstate { q3 = ltq_queue_in_r(MsgStatus, Q3) }
  end.

queue_out(State = #vqstate { mode = default, q4 = Q4 }) ->
  case ltq_queue_out(Q4) of
    {empty, _Q4} ->
...
%% but instead of removing messages based on a ltq_queue, this removes
...
remove_by_predicate(Pred, State = #vqstate {out_counter = OutCount}) ->
  {MsgProps, QAcc, State1} =
    collect_by_predicate(Pred, ltq_queue:new(), State),
  State2 =
  ...
  {MsgProps, maybe_update_rates(
    State2 #vqstate {
      out_counter = OutCount + ltq_queue:len(QAcc)})}.
...
process_queue_entries(Q, Fun, FetchAcc, State) ->
  ltq_queue_foldl(fun (MsgStatus, Acc) ->
  ...
  case queue_out(State) of
    ...
    {{value, MsgStatus = #msg_status { msg_props = MsgProps }}, State1} ->
      case Pred(MsgProps) of
        true  -> collect_by_predicate(Pred, ltq_queue_in(MsgStatus, QAcc),
          State1);
...
purge1(AfterFun, State = #vqstate { q4 = Q4}) ->
  ...
  State2 = #vqstate {q1 = Q1} =
    purge_betas_and_deltas(AfterFun, State1#vqstate{q4 = ltq_queue:new()}),

  State3 = remove_queue_entries(Q1, AfterFun, State2),

  a(State3#vqstate{q1 = ltq_queue:new()}).
...
remove_duplicate(MsgStatus = #msg_status {
  seq_id        = SeqId,
  msg_id        = MsgId,
  is_persistent = IsPersistent,
  is_delivered  = IsDelivered,
...
publish1(Msg = #basic_message { is_persistent = IsPersistent, id = MsgId },
  ...
  { State2, Fl } = case ltq_queue:is_empty(Q3) of
    false ->
      { Flag, NewQ1 } = ltq_queue:in(Q1,  msg_status_key(MsgStatus), m(MsgStatus1)),
      { State1#vqstate{ q1 = NewQ1 }, Flag };
    true  ->
      { Flag, NewQ4 } = ltq_queue:in(Q4,  msg_status_key(MsgStatus), m(MsgStatus1)),
      { State1#vqstate{ q4 = NewQ4 }, Flag }
  end,
  case Fl of
    ok ->
      InCount1 = InCount + 1,
      ...
      stats({1, 0}, {none, MsgStatus1}, 0,
        State2#vqstate{ next_seq_id = SeqId + 1,
          in_counter  = InCount1,
          unconfirmed = UC1 });
    { duplicate, MsgStatusOld} ->
      %%rabbit_log:info("MsgStatusOld ~p", [ MsgStatusOld ]),
      { _, State3 } = remove_pending_ack(false, MsgStatusOld#msg_status.seq_id, State2),
      State3
  end;
...
record_pending_ack(#msg_status { seq_id = SeqId } = MsgStatus,
  ...
  Insert = fun (Tree) ->
    try
      ...
      error:{key_exists,_} ->
        gb_trees:update(SeqId, MsgStatus, Tree);
      E:R ->
        rabbit_log:error("Error for tree ~p ~p ~p ~p ~p", [ E, R, Tree , SeqId, erlang:get_stacktrace()]),
...
lookup_pending_ack(SeqId, #vqstate { ram_pending_ack  = RPA,
  ...
  case gb_trees:lookup(SeqId, RPA) of
    ...
    none       -> case gb_trees:lookup(SeqId, DPA) of
      {value, V} -> V;
      none       -> gb_trees:lookup(SeqId, QPA)
    end
  end.
...
queue_merge(SeqIds, Q, MsgIds, Limit, PubFun, State) ->
  queue_merge(SeqIds, Q, ltq_queue:new(), MsgIds,
    Limit, PubFun, State).

queue_merge([SeqId | Rest] = SeqIds, Q, Front, MsgIds,
    Limit, PubFun, State)
  when Limit == undefined orelse SeqId < Limit ->
  case ltq_queue_out(Q) of
    {{value, #msg_status { seq_id = SeqIdQ } = MsgStatus}, Q1}
      when SeqIdQ < SeqId ->
      %% enqueue from the remaining queue
      queue_merge(SeqIds, Q1, ltq_queue_in(MsgStatus, Front), MsgIds,
        Limit, PubFun, State);
    {_, _Q1} ->
      ...
      case msg_from_pending_ack(SeqId, State) of
        ...
        {MsgStatus, State1} ->
          ...
          queue_merge(Rest, Q, ltq_queue_in(MsgStatus1, Front), [MsgId | MsgIds],
            Limit, PubFun, State2)
      end
  end;
queue_merge(SeqIds, Q, Front, MsgIds,
    _Limit, _PubFun, State) ->
  {SeqIds, ltq_queue:join(Front, Q), MsgIds, State}.
...
reduce_memory_use(State = #vqstate {
  ...
  case chunk_size(ltq_queue:len(Q2) + ltq_queue:len(Q3),
    Permitted) of
...
reduce_memory_use(State = #vqstate {
  ...
  State3 =
    case chunk_size(ltq_queue:len(Q3),
...
permitted_beta_count(#vqstate { target_ram_count = 0, q3 = Q3 }) ->
  lists:min([ltq_queue:len(Q3), rabbit_queue_index:next_segment_boundary(0)]);
permitted_beta_count(#vqstate { q1               = Q1,
  q4               = Q4,
  target_ram_count = TargetRamCount,
  len              = Len }) ->
  BetaDelta = Len - ltq_queue:len(Q1) - ltq_queue:len(Q4),
...
fetch_from_q3(State = #vqstate { mode  = default,
  ...
  q3    = Q3,
  q4    = Q4 }) ->
  case ltq_queue_out(Q3) of
    {empty, _Q3} ->
      {empty, State};
    {{value, MsgStatus}, Q3a} ->
      State1 = State #vqstate { q3 = Q3a },
      State2 = case {ltq_queue:is_empty(Q3a), 0 == DeltaCount} of
        {true, true} ->
          ...
          true = ltq_queue:is_empty(Q2), %% ASSERTION
          true = ltq_queue:is_empty(Q4), %% ASSERTION
          State1 #vqstate { q1 = ltq_queue:new(), q4 = Q1 };
        {true, false} ->
...
maybe_deltas_to_betas(DelsAndAcksFun,
  ...
  case ltq_queue:len(Q3a) of
    ...
    Q3aLen ->
      Q3b = ltq_queue:join(Q3, Q3a),
      case DeltaCount - Q3aLen of
        0 ->
          %% delta is now empty, but it wasn't before, so
          %% can now join q2 onto q3
          State2 #vqstate { q2    = ltq_queue:new(),
            delta = ?BLANK_DELTA,
            q3    = ltq_queue:join(Q3b, Q2),
            delta_transient_bytes = 0};
        N when N > 0 ->
    ...
    push_alphas_to_betas(
      ...
      fun (MsgStatus, Q1a,
          State0 = #vqstate { q3 = Q3, delta = #delta { count = 0,
            transient = 0 } }) ->
        State0 #vqstate { q1 = Q1a, q3 = ltq_queue_in(MsgStatus, Q3) };
        (MsgStatus, Q1a, State0 = #vqstate { q2 = Q2 }) ->
          State0 #vqstate { q1 = Q1a, q2 = ltq_queue_in(MsgStatus, Q2) }
      end, Quota, State #vqstate.q1, State),
  {Quota2, State2} =
    push_alphas_to_betas(
      fun ltq_queue_out_r/1,
      fun (MsgStatus, Q4a, State0 = #vqstate { q3 = Q3 }) ->
        State0 #vqstate { q3 = ltq_queue_in_r(MsgStatus, Q3), q4 = Q4a }
...
push_betas_to_deltas(Generator, LimitFun, Q, PushState) ->
  case ltq_queue:is_empty(Q) of
    true ->
      {Q, PushState};
    false ->
      {value, #msg_status { seq_id = MinSeqId }} = ltq_queue_peek(Q),
      {value, #msg_status { seq_id = MaxSeqId }} = ltq_queue_peek_r(Q),
...
move_messages_to_vhost_store(Queues) ->
  ...
  MigrationBatchSize = application:get_env(rabbit, queue_migration_batch_size,
    ltq_queue_MIGRATION_BATCH_SIZE),
...
ltq_queue_in_r(MsgStatus = #msg_status{}, Q3) ->
  { _, NewQ } = ltq_queue:in_r(Q3, msg_status_key(MsgStatus), MsgStatus),
  NewQ.

ltq_queue_in(MsgStatus = #msg_status{}, Q3) ->
  { _, NewQ } = ltq_queue:in(Q3, msg_status_key(MsgStatus), MsgStatus),
  NewQ.

ltq_queue_out_r(Q) ->
  case ltq_queue:out_r(Q) of
    { empty, _ } = Result -> Result;
    {{_Key, Value}, NewQ} -> {{value, Value}, NewQ }
  end.

ltq_queue_out(Q) ->
  case ltq_queue:out(Q) of
    { empty, _ } = Result -> Result;
    {{_Key, Value}, NewQ} -> {{value, Value}, NewQ }
  end.

ltq_queue_peek(Q) ->
  case ltq_queue:peek(Q) of
    empty = Result -> Result;
    {_Key, Value} -> {value, Value}
  end.

ltq_queue_peek_r(Q) ->
  case ltq_queue:peek_r(Q) of
    empty = Result -> Result;
    {_Key, Value} -> {value, Value}
  end.
...
%%passthrough1(State = #passthrough{ bq = BQ, bq_state = BQState }, FuncAtom, Args) ->
%%  State#passthrough{
%%    bq_state = apply(BQ, FuncAtom, Args )
%%  }.
%%
%%passthrough2(State = #passthrough{ bq = BQ, bq_state = BQState }, FuncAtom, Args) ->
%%  { Res, NewBQState } = apply(BQ, FuncAtom, Args ),
%%  NewState = State#passthrough{
%%    bq_state = NewBQState
%%  },
%%  { Res, NewState}.
%%
%%passthrough3(State = #passthrough{ bq = BQ, bq_state = BQState }, FuncAtom, Args) ->
%%  { Res1, Res2, NewBQState } = apply(BQ, FuncAtom, Args ),
%%  NewState = State#passthrough{
%%    bq_state = NewBQState
%%  },
%%  { Res1, Res2, NewState}.
